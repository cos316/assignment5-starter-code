...
When the application closes a connection, the connection pool adds the connection back to its list of
available connections without actually closing the connection.
...
calls `Open()`. In fact, for this assignment, __increment the total numder of connections in
